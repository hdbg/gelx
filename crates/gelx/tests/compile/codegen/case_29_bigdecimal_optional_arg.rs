pub mod example {
    ...
    pub struct Input {
        #[builder(default, setter(into, strip_option(fallback = amount_opt)))]
    ...
    pub const QUERY: &str = "with amount := <optional decimal>$amount, select Transaction {*} filter (.amount = amount if exists amount else True);";
}
fn main() {}
